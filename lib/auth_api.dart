import 'package:intl/intl.dart';
import 'package:googleapis/sheets/v4.dart' as sheets;
import 'package:googleapis_auth/auth_io.dart';

class AuthApi {
  static const credentials = r'''
    {
      "type": "service_account",
      "project_id": "qrcode-389011",
      "private_key_id": "17f3dbdb5da24f0c91442d9a2b55a343bd3066f8",
      "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
      "client_email": "qrcode@qrcode-389011.iam.gserviceaccount.com",
      "client_id": "106691019817219857675",
      "auth_uri": "https://accounts.google.com/o/oauth2/auth",
      "token_uri": "https://oauth2.googleapis.com/token",
      "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
      "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/qrcode%40qrcode-389011.iam.gserviceaccount.com",
      "universe_domain": "googleapis.com"
    }
  ''';

  static const spreadsheetId = '18MLNYUAaL92hXmOJMq6LXBsjNH2P-k4haeuLvl8tMbw';
  static const range = 'Sheet1!A2:C';

  static Future<String?> login(String username, String password) async {
    final client = await clientViaServiceAccount(
      ServiceAccountCredentials.fromJson(credentials),
      ['https://www.googleapis.com/auth/spreadsheets.readonly'],
    );

    try {
      final sheets.SheetsApi sheetsApi = sheets.SheetsApi(client);

      final sheets.ValueRange response =
      await sheetsApi.spreadsheets.values.get(spreadsheetId, range);

      final List<List<dynamic>> values = response.values as List<List<dynamic>>;

      for (final List<dynamic> row in values) {
        final storedUsername = row[0] as String;
        final storedPassword = row[1] as String;
        final role = row[2] as String;

        if (storedUsername == username && storedPassword == password) {
          return role;
        }
      }
    } catch (e) {
      print('Error retrieving data from Google Sheets: $e');
    } finally {
      client.close();
    }

    return null; // Return null if the username/password is not found or there is an error.
  }

}
